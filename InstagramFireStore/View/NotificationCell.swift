...
class NotificationCell: UITableViewCell {
    ...
    @objc func handleFollowTapped() {
        ...
        if viewModel.notification.userIsFollowed {
            delegate?.cell(self, wantsToUnfollowUserWithUid: viewModel.notification.uid)
        } else {
            delegate?.cell(self, wantsToFollowUserWithUid: viewModel.notification.uid)
        }
    }
    
    @objc func handlePostTapped() {
        guard let postId = viewModel?.notification.postId else { return }
        delegate?.cell(self, wantsToViewPostWithPostId: postId)
    }
